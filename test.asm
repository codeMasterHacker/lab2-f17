...
int main(int argc, char** argv)
{
    ...
    printf(1, "%p\n", &argc);
    1011:	51                   	push   %ecx
    1012:	68 24 17 00 00       	push   $0x1724
    1017:	6a 01                	push   $0x1
    1019:	e8 b2 03 00 00       	call   13d0 <printf>
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    ...
    136f:	f7 f6                	div    %esi
    1371:	0f b6 92 30 17 00 00 	movzbl 0x1730(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    14e8:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
    14eb:	b8 28 17 00 00       	mov    $0x1728,%eax
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    1571:	a1 08 1a 00 00       	mov    0x1a08,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    15c9:	a3 08 1a 00 00       	mov    %eax,0x1a08
}
    15ce:	5b                   	pop    %ebx
    15cf:	5e                   	pop    %esi
    15d0:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    15ed:	a3 08 1a 00 00       	mov    %eax,0x1a08
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    160c:	8b 15 08 1a 00 00    	mov    0x1a08,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    1661:	39 05 08 1a 00 00    	cmp    %eax,0x1a08
  ...
  return freep;
    168b:	8b 15 08 1a 00 00    	mov    0x1a08,%edx
      }
      freep = prevp;
      return (void*)(p + 1);
    }
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    16ac:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
    16af:	89 15 08 1a 00 00    	mov    %edx,0x1a08
      return (void*)(p + 1);
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    16c6:	c7 05 08 1a 00 00 0c 	movl   $0x1a0c,0x1a08
    16cd:	1a 00 00 
    16d0:	c7 05 0c 1a 00 00 0c 	movl   $0x1a0c,0x1a0c
    16d7:	1a 00 00 
    base.s.size = 0;
    16da:	b8 0c 1a 00 00       	mov    $0x1a0c,%eax
    16df:	c7 05 10 1a 00 00 00 	movl   $0x0,0x1a10
    16e6:	00 00 00 
